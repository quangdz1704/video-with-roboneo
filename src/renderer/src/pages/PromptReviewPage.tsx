    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold">Review prompt</h1>
          <p className="mt-2 text-muted-foreground">
            Edit the exact prompt sent to RoboNeo.
          </p>
        </div>
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() => navigate(`/new?project=${project.id}`)}
          >
            <ArrowLeft className="h-4 w-4" /> Back
          </Button>
          <Button disabled={busy || !prompt.trim()} onClick={() => void run()}>
            <Play className="h-4 w-4" /> Run RoboNeo
          </Button>
        </div>
      </div>
      <PromptReview
        pack={project.promptPack}
        finalPrompt={prompt}
        onChange={setPrompt}
      />
    </div>
