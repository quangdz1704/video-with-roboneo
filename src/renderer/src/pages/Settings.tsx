...
export function Settings(): JSX.Element {
  ...
  const [environment, setEnvironment] =
    useState<Awaited<ReturnType<typeof window.roboneo.checkEnvironment>>>();
  useEffect(() => {
    void window.roboneo.checkEnvironment().then(setEnvironment);
  }, []);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">
          Local CLI, polling, and output defaults.
        </p>
      </div>
      <Card>
        <CardHeader>
          ...
          <div className="flex items-center gap-3 rounded-lg border border-border p-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {environment?.node.version || "Not detected"}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3 rounded-lg border border-border p-4">
            ...
            <div>
              <p className="font-medium">RoboNeo CLI</p>
              <p className="text-xs text-muted-foreground">
                {environment?.cli.version ||
                  `Missing · ${environment?.cli.installCommand || "npm install -g roboneo-cli"}`}
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="grid gap-5 md:grid-cols-2">
          <label className="space-y-2">
            <span className="text-sm font-medium">RoboNeo CLI path</span>
            <Input
          ...
          <label className="space-y-2">
            <span className="text-sm font-medium">Poll interval (ms)</span>
            <Input
              type="number"
              min={3000}
              value={settings.pollIntervalMs}
              onChange={(e) =>
                patch({ pollIntervalMs: Number(e.target.value) })
              }
            />
          </label>
          <label className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) =>
                patch({ defaultDuration: Number(e.target.value) })
              }
            />
          </label>
          <label className="space-y-2">
            ...
            <select
              ...
              onChange={(e) =>
                patch({ defaultLanguage: e.target.value as "en" | "vi" })
              }
            >
      ...
      <div className="flex justify-end">
        <Button
          onClick={async () => {
            const saved = await window.roboneo.saveSettings(settings);
