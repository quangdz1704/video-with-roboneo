...
export function NewVideo(): JSX.Element {
  ...
  const patch = (value: Partial<Project>): void => {
    ...
    if (!current) return;
    const changed = Object.entries(value).some(
      ([key, nextValue]) => current[key as keyof Project] !== nextValue,
    );
    if (!changed) return;
  ...
  const selectMode = (nextMode: GenerationMode): void =>
    patch({ mode: nextMode });
  const chooseAsset = async (
    slot: "characterImage" | "secondImage" | "referenceVideo",
    kind: "image" | "video",
  ): Promise<void> => {
    const current = projectRef.current;
    if (!current) return;
    ...
    const selected = await window.roboneo.selectAsset({
      projectId: current.id,
      slot,
      kind,
    });
    if (selected)
      patch({ assets: { ...projectRef.current!.assets, [slot]: selected } });
  };
  const review = async (): Promise<void> => {
    ...
    const saved = await window.roboneo.updateProject({
      ...projectRef.current!,
      promptPack,
      finalPrompt: promptPack.finalPrompt,
    });
    upsertProject(saved);
    setBusy(false);
  ...
  const modeOptions: Array<{
    ...
    },
    {
      id: "text_to_video",
      label: "Text to Video",
      description: "Generate a video without reference assets",
      icon: Film,
    },
    {
      id: "image_to_video",
  ...
  return (
    <div className="space-y-7">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-semibold">Create with RoboNeo</h1>
        <p className="mt-2 text-muted-foreground">
          Choose a generation workflow. Local assets are copied into the project
          input folder.
        </p>
      </div>
      {saveError ? (
        ...
        {modeOptions.map((option) => {
          ...
          return (
            <button
              ...
              className={`rounded-xl border p-4 text-left transition ${mode === option.id ? "border-primary bg-primary/10" : "border-border bg-card hover:border-primary/50"}`}
            >
              <Icon
                className={`mb-3 h-5 w-5 ${mode === option.id ? "text-primary" : "text-muted-foreground"}`}
              />
      ...
      {mode === "motion_reference" ? (
        <div className="grid gap-4 lg:grid-cols-3">
          <AssetPreview
            ...
            onSelect={() => void chooseAsset("characterImage", "image")}
          />
          <AssetPreview
            ...
            onSelect={() => void chooseAsset("secondImage", "image")}
          />
          <AssetPreview
            label="Motion reference"
            type="video"
            path={project.assets.referenceVideo}
            onSelect={() => void chooseAsset("referenceVideo", "video")}
          />
        </div>
      ) : null}
      {mode === "image_to_video" ? (
      ...
      {mode === "text_to_image" || mode === "text_to_video" ? (
        <Card>
          <CardContent className="p-5 text-sm text-muted-foreground">
            This workflow does not require an uploaded asset. RoboNeo will
            generate directly from the final prompt.
          </CardContent>
        </Card>
      ) : null}
      <Card>
        ...
        <CardContent className="grid gap-5 md:grid-cols-2">
          ...
          <label className="space-y-2 md:col-span-2">
            ...
            <Textarea
              ...
              placeholder={
                mode === "text_to_image"
                  ? "Describe subject, composition, lighting, style, colors, and intended use..."
                  : "Describe the scene, action, camera, product, audience, and desired ending..."
              }
            />
          </label>
          {isVideoMode && (
      ...
      <div className="flex justify-end">
        <Button
          size="lg"
          disabled={
            busy || !assetsValid || !project.brief.trim() || !project.apiKeyId
          }
          onClick={() => void review()}
        >
          <Sparkles className="h-4 w-4" /> Generate prompt pack{" "}
