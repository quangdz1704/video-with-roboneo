...
export function generatePromptPack(input: PromptInput): PromptPack {
  ...
  const tiktok = mode === 'text_to_image'
  ...
  const taskIntro: Record<GenerationMode, string> = {
    motion_reference: 'Create a vertical 9:16 TikTok-ready AI video using the provided character reference image and motion reference video.',
    text_to_image: 'Create a high-quality vertical 9:16 TikTok-ready image from the text description. Do not create a video.',
    text_to_video: 'Create a vertical 9:16 TikTok-ready AI video directly from the text description. No reference asset is required.',
    image_to_video: 'Animate the provided reference image into a vertical 9:16 TikTok-ready AI video while preserving its subject and visual identity.'
  }

  const output = mode === 'text_to_image'
    ? 'Vertical 9:16, 1080x1920, high-resolution JPG or PNG.'
    : `Vertical 9:16, 1080x1920, ${input.duration} seconds, MP4-ready.`

  const referenceSection = mode === 'motion_reference'
  ...
  const style = mode === 'text_to_image'
  ...
  const variations = mode === 'text_to_image'
    ? [
...
${output}`
  ...
  return {
    ...
    variations: variations.map((variation) => `${mainPrompt}\n\nVariation: ${variation}`),
